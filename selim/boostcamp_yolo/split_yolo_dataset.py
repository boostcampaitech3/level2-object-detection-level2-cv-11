...
if __name__ == '__main__':
    ...
    use_folds = [3] #[0,2,4]   # 10Fold중 0,2,4 Fold만 사용함. (학습 시간 때문에)
    for fold, (trn_idx, val_idx) in enumerate(kf.split(full_images)):
        ...
        copy_files(train_images, prefix=f'../dataset/yolo/{fold}fold/train/images/')
        copy_files(train_labels, prefix=f'../dataset/yolo/{fold}fold/train/labels/')
        copy_files(val_images, prefix=f'../dataset/yolo/{fold}fold/valid/images/')
        copy_files(val_labels, prefix=f'../dataset/yolo/{fold}fold/valid/labels/')
